...
class LocalLLMClient:
    def __init__(self, model="llama3.2:3b"):
        ...
        self.model = model  # デフォルトは軽量なllama3.2:3b
        ...
        if not self.check_ollama_connection():
            ...
            print("インストール: curl -fsSL https://ollama.com/install.sh | sh")
            print("モデル取得: ollama pull llama3.2:3b")
...
練習問題:
"""
        return self._send_message(prompt, system_message="あなたは教育問題作成の専門家です。適切な難易度で学習効果の高い問題を作成します。")
...
説明:
"""
        return self._send_message(prompt, system_message="あなたは教育専門家です。複雑な概念を分かりやすく説明することが得意です。")
...
学習計画:
"""
        return self._send_message(prompt, system_message="あなたは学習計画の専門家です。効果的で実践的な学習方法を提案します。")
...
class AIProcessingDialog:
    ...
    def __init__(self, parent, ocr_text, subject_name=""):
        ...
        self.llm_client = LocalLLMClient(model="gemma2:2b")  # より良い日本語対応
    ...
    def save_result(self, result):
        ...
        try:
            # 保存フォルダを作成
            ai_results_dir = os.path.join(os.path.dirname(__file__), 'ai_results')
            os.makedirs(ai_results_dir, exist_ok=True)
            
            # ファイル名を生成
            timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
            filename = os.path.join(ai_results_dir, f'ai_result_{timestamp}.txt')
